...
# Verifies that skills are resolved with correct priority: project > personal > Superpowers-zh
...
# 1. Create in Superpowers-zh location (lowest priority)
mkdir -p "$HOME/.config/opencode/Superpowers-zh/skills/priority-test"
cat > "$HOME/.config/opencode/Superpowers-zh/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superpowers-zh version of priority test skill
---
# Priority Test Skill (Superpowers-zh Version)

This is the Superpowers-zh version of the priority test skill.
...
if [ -f "$HOME/.config/opencode/Superpowers-zh/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superpowers-zh version exists"
else
    echo "  [FAIL] Superpowers-zh version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides Superpowers-zh
echo ""
echo "Test 2: Testing personal > Superpowers-zh priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides Superpowers-zh)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] Superpowers-zh version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|Superpowers-zh" | head -10
fi

# Test 3: Test that project overrides both personal and Superpowers-zh
echo ""
echo "Test 3: Testing project > personal > Superpowers-zh priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Superpowers-zh version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit superpowers-zh: prefix bypasses priority
echo ""
echo "Test 4: Testing superpowers-zh: prefix forces Superpowers-zh version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load superpowers-zh:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] superpowers-zh: prefix correctly forces Superpowers-zh version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] superpowers-zh: prefix did not force Superpowers-zh version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
