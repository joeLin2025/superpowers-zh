...
### 2. 必须创建标准化计划文件 (Plan Output Standard)
- **路径**：`docs/plans/[YYYY-MM-DD]-[task-name].md` (若目录不存在，自动创建)。
- **Schema**：
...
### 3. 必须执行“原子化”拆解
- **粒度要求**：每个 Task 的代码改动不应超过 50 行（除样板代码外）。
- **验证强制**：每个 Task 下必须显式包含 `验证` 子项。没有验证步骤的计划是无效的。
