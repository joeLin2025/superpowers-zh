...
### 2. 必须提供结构化描述 (Structured Description)
发起请求时，**必须**使用中文提供包含以下要素的描述（严禁“Fix bug”一句话总结）：
- **动机 (Why)**：为什么要进行此项改动？关联的 Issue 或需求是什么？
- **核心变更 (What)**：高层逻辑有哪些变化？新增了哪些核心组件？
- **风险点 (Risks)**：这次改动是否会影响性能？是否涉及数据库迁移？是否有破坏向后兼容性的风险？
- **验证证据 (Verification)**：贴出关键测试通过的日志片段或执行结果。
