...
### 2. 必须强制执行“六段式”标准结构
所有技能文档**必须**包含以下一级标题，顺序严禁颠倒：
- **Frontmatter**：YAML 格式，定义触发场景。
- **概述 (Overview)**：定义技能的“灵魂”与核心原则。
- **何时使用 (When to use)**：精确的触发条件与**禁忌场景**。
- **核心法则 (Core Rules)**：以“必须/严禁”开头的绝对命令。
- **借口粉碎机 (Excuse Smasher)**：预判偷懒思维并进行降维打击。
- **危险信号 (Red Flags)**：定义任务的“熔断点”。
