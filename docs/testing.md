...
本文档描述如何测试 Superpowers 技能，特别是针对复杂技能（如 `subagent-driven-development`）的集成测试。
...
- 本地开发市场必须在 `~/.claude/settings.json` 中启用：`"superpowers@superpowers-dev": true`
...
4. **从插件目录运行**：技能仅在从 superpowers 目录运行时加载
